...
from python_models8.model_data_holders.my_model_cond_exp_data_holder \
    import MyModelCondExpDataHolder as My_Model_Cond_Exp
...
myModelCondExpParams = {
    "my_parameter": -70.0,
    "i_offset": i_offset
}
my_model_cond_pop = p.Population(
    1, My_Model_Cond_Exp(**myModelCondExpParams),
    label="my_model_cond_pop")
p.Projection(
    input_pop, my_model_cond_pop,
    p.OneToOneConnector(), receptor_type='excitatory',
    synapse_type=p.StaticSynapse(weight=weight_cond))
...
v_my_model_cond_pop = my_model_cond_pop.get_data('v')
...
Figure(
    ...
    Panel(v_my_model_cond_pop.segments[0].filter(name='v')[0],
          ylabel="Membrane potential (mV)",
          data_labels=[my_model_cond_pop.label],
